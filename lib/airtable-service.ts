...
export class AirtableService {
  ...
  /** True when AIRTABLE_BASE_ID and AIRTABLE_PERSONAL_ACCESS_TOKEN are set. */
  isConfigured(): boolean {
    return Boolean(this.baseId && this.token)
  }
  ...
  async createUserRecord(data: Omit<UserRecord, "Status" | "User_ID">): Promise<string | null> {
    if (!this.isConfigured()) {
      throw new Error("AIRTABLE_NOT_CONFIGURED")
    }

    const { Referral_Name, Created_At, ...core } = data
    ...
    if (Referral_Name?.trim()) {
      fields.Referral_Name = Referral_Name.trim()
    }
  ...
  async getUserByRecordId(recordId: string): Promise<{ id: string; fields: UserRecord } | null> {
    if (!this.baseId || !this.token) return null
    try {
      const response = await fetch(`${this.usersBaseUrl}/${recordId}`, {
        headers: this.getHeaders(),
      })
      const data = await this.handleAirtableResponse(response)
      return { id: data.id, fields: data.fields as UserRecord }
    } catch (error) {
      console.error("Error fetching user by record id:", error)
      return null
    }
  }

  async getUsersByUserId(userId: string): Promise<any[]> {
    try {
      const safe = escapeAirtableString(userId)
      const response = await fetch(
        `${this.usersBaseUrl}?filterByFormula={User_ID}='${safe}'&maxRecords=1`,
        { headers: this.getHeaders() }
      )
      const data = await this.handleAirtableResponse(response)
      return data.records || []
  ...
  async getUsersByEmail(email: string): Promise<any[]> {
    try {
      const safe = escapeAirtableString(email.trim().toLowerCase())
      const response = await fetch(
        `${this.usersBaseUrl}?filterByFormula=LOWER({Email})='${safe}'&maxRecords=5`,
        { headers: this.getHeaders() }
      )
  ...
  async getUsersByUsername(username: string): Promise<any[]> {
    try {
      const safe = escapeAirtableString(username.trim())
      const response = await fetch(
        `${this.usersBaseUrl}?filterByFormula={Username}='${safe}'&maxRecords=5`,
        { headers: this.getHeaders() }
      )
  ...
  async getUsersByResetToken(resetToken: string): Promise<any[]> {
    try {
      const safe = escapeAirtableString(resetToken)
      const response = await fetch(
        `${this.usersBaseUrl}?filterByFormula={reset_token}='${safe}'&maxRecords=1`,
        { headers: this.getHeaders() }
      )
